...
SELECT COUNT(*) AS "TOTAL CUSTOMERS", cus_gender 
from customer
where customer.cus_id in(SELECT distinct(cus_id) 
						   from `order` 
...
select category.CAT_ID,CAT_NAME, min(supplier_pricing.SUPP_PRICE)
  from supplier_pricing
